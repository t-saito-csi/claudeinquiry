...
erDiagram
    ...
    staff_users         ||--o{ inquiry_sessions   : "担当医"
...
CREATE TABLE inquiry_sessions (
    ...
    doctor_id          UUID,                               -- 担当医（staff_users, doctor ロール、NULL可）
    created_by         UUID           NOT NULL REFERENCES staff_users(id),
    created_at         TIMESTAMPTZ    NOT NULL DEFAULT NOW(),
    updated_at         TIMESTAMPTZ    NOT NULL DEFAULT NOW(),
...
| doctor_id | UUID | NULL | - | FK → staff_users.id（担当医。NULL可） |
...
| inquiry_sessions | doctor_id | staff_users.id | SET NULL |
